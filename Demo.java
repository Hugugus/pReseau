...
import reso.examples.queueing.AppPacketSink;
...
import static reso.examples.queueing.Demo.*;
...
public class Demo
{
    ...
    public static void main(String [] args) {
		...
		try {
			...
			host1= NetworkBuilder.createHost(network, "H1", IP_ADDR1, MAC_ADDR1);
			host1.getIPLayer().addRoute(IP_ADDR2, "eth0");
			if (ENABLE_SNIFFER)
				host1.addApplication(new App(host1));
			host1.addApplication(new App(host1));

			host2= NetworkBuilder.createHost(network,"H2", IP_ADDR2, MAC_ADDR2);
			host2.getIPLayer().addRoute(IP_ADDR1, "eth0");
			host2.addApplication(new App(host2));
	...
	public static IPHost host1;
	public static IPHost host2;
